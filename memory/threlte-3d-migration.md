...
Staged plan: **0+1 DONE** (deps, ground+grid+track meshes, perspective camera w/ OrbitControls, raycast place/erase/rotate/station/loco/decorate; `draw` tool stubs a straight on click). **2 DONE** (trees=trunk+2 cones, buildings=box+pyramid-cone roof, water plane, grass/stone groundovers, stations=platform+shelter+people; all cast/receive shadows; reuses the same `decorations.ts` spot helpers as SVG). **3** locos/wagons/particles driven from `sim`. **4** parity (drag-draw, switch shift-toggle polish, dark-mode lighting, camera pan) + perf (instancing for dense decoration — currently ~24 meshes per tree tile). **5** decide default vs toggle, and whether to remove the dormant SVG tilt code.
